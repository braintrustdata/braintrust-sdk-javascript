...
/**
 * Convert a Fetch API `Request` to a `RecordedRequest`.
 *
...
/**
 * Convert a Fetch API `Request` to a `RecordedRequestOrDraft` for record mode.
 *
...
/**
 * Convert a Fetch API `Response` to a `RecordedResponseOrDraft` for record mode.
 *
...
/** Build a Fetch API `Response` from a recorded response body. */
export async function buildResponse(
  recorded: RecordedResponse | RecordedResponseOrDraft,
  ...
  // layer (undici decompresses gzip/deflate before handing the body to us).
  // Preserving content-encoding would cause callers to attempt a second
  // decode of already-plain bytes, which throws a zlib error.
  ...
  const bytes =
    recorded.body.kind === "binary-draft"
      ? recorded.body.bytes
      : await decodeBody(recorded.body, ctx);
