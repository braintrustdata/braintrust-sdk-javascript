...
class FlueInstrumentationConsumer {
  public register(): void {
    enableFlueAutoInstrumentation();
  }
}

function enableFlueAutoInstrumentation(): void {
  if (isAutoState(Reflect.get(globalThis, FLUE_AUTO_STATE))) {
    return;
  }
...
function isAutoState(value: unknown): value is FlueAutoState {
  return (
    isObjectLike(value) && Reflect.get(value, "contexts") instanceof WeakSet
  );
}
...
let flueInstrumentationConsumer: FlueInstrumentationConsumer | undefined;

export function registerFlueInstrumentation(): void {
  flueInstrumentationConsumer ??= new FlueInstrumentationConsumer();
  flueInstrumentationConsumer.register();
}
